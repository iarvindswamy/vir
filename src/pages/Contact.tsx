...
export function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("https://formspree.io/f/xykaqlzw", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          _subject: `New Contact Form: ${formData.firstName} ${formData.lastName} from ${formData.company}`
        })
      });
      ...
      setError("Something went wrong. Please email us directly at contact@virtechnologies.in");
    } finally {
      setSubmitting(false);
    }
  };
                ...
                {submitted ? (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <button
                      type="submit"
                      disabled={submitting}
                      className="inline-flex items-center gap-2 bg-electric-teal hover:bg-electric-teal/90 text-white px-8 py-4 rounded font-medium transition-all hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {submitting ? (
                        <>
